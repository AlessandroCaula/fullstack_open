...
const App = () => {
  ...
  // useQuery hook for fetching the data from the server
  ...
  // --- Retrieve the bloglist from the server with the React useQuery
  // 
  const result = useQuery({
    queryKey: ['blogs'],
    queryFn: blogService.getAll,
    retry: 10,    // Retry the fetch request only once
    enabled: !user
  })
  // If the query is still loading state
  if (result.isPending) {
  ...
  // --- Add new blog with a new mutation
  //
  const newBlogMutation = useMutation({
    mutationFn: blogService.create,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['blogs'] })  // Invalidate in order to re-render the blogs
    }
  })
